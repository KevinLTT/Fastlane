...
public abstract class ForwardingBase implements IOFMessageListener {
			...
			boolean requestFlowRemovedNotification, OFFlowModCommand flowModCommand) 
	{
		...
		boolean packetOutSent = false; //发送成功标志

		List<NodePortTuple> switchPortList = route.getPath(); //从route中获取交发送端口队列

		if( flowModCommand != OFFlowModCommand.ADD ) //不是添加规则, 按原本步骤执行
		{
		...
		{
			DatapathId switchDPID = switchPortList.get( 0 ).getNodeId(); //获取入口交换机DPID
			IOFSwitch sw = switchService.getSwitch( switchDPID ); //根据DPID获取入口交换机信息
			if( sw == null ) //交换机信息获取失败, 记录信息并返回 
			{
				if( log.isWarnEnabled() )
				{
					log.warn("Unable to push route, switch at DPID {} " + "not available", switchDPID);
			...
			OFFlowMod.Builder fmb; //flowmod消息
			fmb = sw.getOFFactory().buildFlowAdd(); 

			List<OFAction> actions = new ArrayList<OFAction>();	//action队列
 			Match.Builder mb = MatchUtils.convertToVersion(match, sw.getOFFactory().getVersion()); //match信息

 			for (int indx = 1; indx < switchPortList.size(); indx += 2) //从头至尾遍历端口队列, 构造各个switch所需要的action并将其添加到action队列中
 			{
 				OFPort outPort = switchPortList.get(indx).getPortId();
 				OFActionOutput.Builder aob = sw.getOFFactory().actions().buildOutput(); //action
 				aob.setPort(outPort);
				aob.setMaxLen(Integer.MAX_VALUE);
				actions.add(aob.build());
 			}

 			if (FLOWMOD_DEFAULT_SET_SEND_FLOW_REM_FLAG || requestFlowRemovedNotification) { //暂时不懂其意图
					Set<OFFlowModFlags> flags = new HashSet<>();
					flags.add(OFFlowModFlags.SEND_FLOW_REM);
					fmb.setFlags(flags);
				}
				
			fmb.setMatch(mb.build()) //设定flowmod的match域, 优先级等信息
			.setIdleTimeout(FLOWMOD_DEFAULT_IDLE_TIMEOUT)
			.setHardTimeout(FLOWMOD_DEFAULT_HARD_TIMEOUT)
			.setBufferId(OFBufferId.NO_BUFFER)
			.setCookie(cookie)
			...
			FlowModUtils.setActions(fmb, actions, sw); //设定flowmod的action域
			...
			try {
				...
				*/
				...
				if (sw.getId().equals(pinSwitch) &&
						...
						!fmb.getCommand().equals(OFFlowModCommand.DELETE_STRICT)) {
					/* Use the buffered packet at the switch, if there's one stored */
					pushPacketModified(sw, pi, outPort, true, cntx, actions);
					packetOutSent = true;
				}
			} catch (IOException e) {
				log.error("Failure writing flow mod", e);
	...
	protected void pushPacketModified(IOFSwitch sw, OFPacketIn pi, OFPort outport, boolean useBufferedPacket, FloodlightContext cntx, List<OFAction> actions) 
	{
		if (pi == null) {
			return;
		}
