...
/* Must be called with rcu_read_lock. */
//key值是从数据包中提取出来的, 用于流表匹配
void ovs_dp_process_packet(struct sk_buff *skb, struct sw_flow_key *key)
{
	const struct vport *p = OVS_CB(skb)->input_vport; //入口端口
	struct datapath *dp = p->dp; //网桥变量, 获取端口所在的网桥指针
	struct sw_flow *flow; //流表
	struct sw_flow_actions *sf_acts; //action
	struct dp_stats_percpu *stats; //对给定的数据包处理统计, 如匹配成功的包的个数等
	u64 *stats_counter;
	u32 n_mask_hit;
	...
	flow = ovs_flow_tbl_lookup_stats(&dp->table, key, skb_get_hash(skb),
					 &n_mask_hit);
	if (unlikely(!flow)) 
	//判断是否匹配到相应的流表项, 如没有则执行下面代码
	{
		struct dp_upcall_info upcall; //定义upcall结构体, 把数据包发送到用户空间
		int error;

		//根据dp_upcall_info数据结构, 对成员进行填充
		memset(&upcall, 0, sizeof(upcall)); //首先数据全部初始化为0
		upcall.cmd = OVS_PACKET_CMD_MISS; //流表匹配失败命令
		upcall.portid = ovs_vport_find_upcall_portid(p, skb); //netLink通信时的id号
		upcall.mru = OVS_CB(skb)->mru;
		error = ovs_dp_upcall(dp, skb, key, &upcall, 0); //把数据包发送到用户空间
		if (unlikely(error))
			kfree_skb(skb);
		else
			consume_skb(skb);
	...
	ovs_execute_actions(dp, skb, sf_acts, key); //根据匹配到的流表项执行相应的操作
				  ...
				  uint32_t cutlen)
{
	...
	*/
	...
	size_t len;
	unsigned int hlen;
	int err, dp_ifindex; //dp_ifindex: 设备结构索引号
