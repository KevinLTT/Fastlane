...
static enum ofperr
fastlane_ofpacts_decode(const void *actions, size_t actions_len,
               enum ofp_version ofp_version, struct ofpbuf *ofpacts,
               struct ofpbuf *remaining_actions)
{
    ...
    while (openflow.size) 
    {
        ...
        error = ofpact_pull_raw(&openflow, ofp_version, &raw, &arg);
        if (!error) {
            error = ofpact_decode(action, raw, ofp_version, arg, remaining_actions);
...
{
    //FILE* fp;
    //fp = fopen( "/home/mininet/testOutput/ofpacts_decode_test.txt", "a" );
    //int mycount = 0;
...
enum ofperr
fastlane_ofpacts_pull_openflow_instructions(struct ofpbuf *openflow,
                                   ...
                                   struct ofpbuf *remaining_actions)
{
    ...
    if (insts[OVSINST_OFPIT11_APPLY_ACTIONS]) {
        ...
        get_actions_from_instruction(insts[OVSINST_OFPIT11_APPLY_ACTIONS],
                                     &actions, &actions_len);
        error = fastlane_ofpacts_decode(actions, actions_len, version, ofpacts, remaining_actions);
        if (error) {
            goto exit;
        }
    }
