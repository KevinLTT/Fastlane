...
enum upcall_type {
    ...
    //PATHCONFIG_UPCALL           /* pathconfig packet */
};
...
classify_upcall(enum dpif_upcall_type type, const struct nlattr *userdata, const struct flow* flow)
{
               ...
               struct ofpbuf *odp_actions, struct flow_wildcards *wc)
{
    ...
    struct dp_packet *packet = dp_packet_clone(upcall->packet);
    const struct flow *flow = upcall->flow;

    switch (classify_upcall(upcall->type, userdata, upcall->flow)) {

    case MISS_UPCALL:
    if( upcall->flow->nw_src == 0x01010101 && upcall->flow->nw_dst == upcall->flow->nw_src )
      {

        /*
        get flow mod from packet
        mac frame header size: 14
        IP packet header size: 20
        UDP segment header size: 8
        total size: 42 
        */
        uint8_t* packet_data = (uint8_t*)packet->base_+packet->data_ofs;
        packet_data += 42;

        struct ofpbuf* flowmod_msg; 
        flowmod_msg = ofpbuf_new( 1536 );
        ofpbuf_put( flowmod_msg, packet_data, packet->size_ - 42 ); 

        //get ofproto
        struct ofproto_dpif *of = upcall->ofproto;
        struct ofproto ofp = of->up;
        
        if( connmgr_has_controllers( ofp.connmgr ) )
        {
          //get ofconn
          struct connmgr* mgr = ofp.connmgr;
          struct ofconn *ofconn = connmgr_get_ofconn(mgr); 

          //handle the flowmod
          fastlane_handle_flow_mod( ofconn, flowmod_msg );
        }
...
handle_upcalls(struct udpif *udpif, struct upcall *upcalls,
               size_t n_upcalls)
{
    //*2是因为先安装规则再对packet进行操作, 分两步走
    struct dpif_op *opsp[UPCALL_MAX_BATCH * 2]; //dpif操作类型 包括flow_put, flow_del, flow_get, execute
    struct ukey_op ops[UPCALL_MAX_BATCH * 2]; //带有ukey的dpif操作
    ...
    for (i = 0; i < n_upcalls; i++) 
    {
        ...
        if (should_install_flow(udpif, upcall))  //需要安装规则
        {
            struct udpif_key *ukey = upcall->ukey;

            upcall->ukey_persists = true;
            op = &ops[n_ops++]; //n_ops需要执行的操作加一

            op->ukey = ukey;
            op->dop.type = DPIF_OP_FLOW_PUT; //操作类型为安装规则
            ...
            ukey_get_actions(ukey, &op->dop.u.flow_put.actions,
                             &op->dop.u.flow_put.actions_len); //从ukey中提取actions至op
        }

        if (upcall->odp_actions.size) //动作存在
        {
            op = &ops[n_ops++]; //n_ops需要执行的操作加一
            op->ukey = NULL;
            op->dop.type = DPIF_OP_EXECUTE; //操作类型为对upcall所带的packet执行操作
            op->dop.u.execute.packet = CONST_CAST(struct dp_packet *, packet); //将upcall中的packet赋给op
            op->dop.u.execute.flow = upcall->flow; //将upcall中的flow赋给op
            odp_key_to_pkt_metadata(upcall->key, upcall->key_len,
                                    &op->dop.u.execute.packet->md); //将upcall中的key作为metadata赋给op
            op->dop.u.execute.actions = upcall->odp_actions.data; //将actions赋给op
            op->dop.u.execute.actions_len = upcall->odp_actions.size; //将actions_len赋给op
    ...
    dpif_operate(udpif->dpif, opsp, n_opsp); //真正执行函数
    for (i = 0; i < n_ops; i++) {
        if (ops[i].ukey) {
