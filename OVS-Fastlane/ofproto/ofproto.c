...
static void fastlane_run_rule_executes( struct ofproto* ofproto, 
                                        struct rule* rule, 
                                        struct flow* flow, 
                                        struct dp_packet* packet )OVS_EXCLUDED(ofproto_mutex);
...
static enum ofperr ofproto_flow_mod_init(struct ofproto *,
    ...
    OVS_EXCLUDED(ofproto_mutex);
static enum ofperr fastlane_ofproto_flow_mod_init(struct ofproto *,
                                         struct ofproto_flow_mod *,
                                         const struct ofputil_flow_mod *fm,
                                         struct rule** new_rule)
    OVS_EXCLUDED(ofproto_mutex);
...
static enum ofperr handle_flow_mod__(struct ofproto *,
                                     ...
                                     const struct openflow_mod_requester *)
    OVS_EXCLUDED(ofproto_mutex);
static enum ofperr fastlane_handle_flow_mod__(struct ofproto *,
                                     const struct ofputil_flow_mod *,
                                     struct rule**)
    OVS_EXCLUDED(ofproto_mutex);
...
static void
fastlane_run_rule_executes( struct ofproto* ofproto, struct rule* rule, struct flow* flow, struct dp_packet* packet )
    OVS_EXCLUDED(ofproto_mutex)
{
    ofproto->ofproto_class->rule_execute(rule, flow, packet);
}
...
static enum ofperr
fastlane_add_flow_init(struct ofproto *ofproto, struct ofproto_flow_mod *ofm,
              const struct ofputil_flow_mod *fm, struct rule** new_rule)
    OVS_EXCLUDED(ofproto_mutex)
{
    struct oftable *table;
    struct cls_rule cr;
    ...
    error = ofproto_rule_create(ofproto, &cr, table - ofproto->tables,
                                fm->new_cookie, fm->idle_timeout,
                                fm->hard_timeout, fm->flags, fm->importance,
                                fm->ofpacts, fm->ofpacts_len, new_rule);
    if (error) {
        return error;
    }
...
/*
struct _MAC_FRAME_HEADER
{
 uint8_t m_cDstMacAddress[6];    //目的mac地址
 uint8_t m_cSrcMacAddress[6];    //源mac地址
 uint16_t m_cType;       　　　　　//上一层协议类型，如0x0800代表上一层是IP协议，0x0806为arp
};
...
struct _MAC_FRAME_HEADER
{
    uint8_t m_cDstMacAddress[6];
    uint8_t m_cSrcMacAddress[6];
    uint16_t m_cType;
};
...
struct ofpbuf*
construct_mac_msg( const struct _MAC_FRAME_HEADER head )
{
    struct ofpbuf *mac_msg = ofpbuf_new( 1536 );

    mac_msg->size += 14;
    uint8_t *msg_head = mac_msg->data;
    uint16_t *msg_head16;

    for( int i = 0; i < 6; i++ )
    {
        *msg_head = head.m_cDstMacAddress[i];
        msg_head++;
    }  
    for( int i = 0; i < 6; i++ )
    {
        *msg_head = head.m_cSrcMacAddress[i];
        msg_head++;
    } 
    msg_head16 = msg_head;
    *msg_head16 = htons( head.m_cType );

    return mac_msg;
}

struct ofpbuf*
construct_ip_msg( const struct _IP_HEADER head )
{
    struct ofpbuf *ip_msg = ofpbuf_new( 1536 );

    ip_msg->size += 20;
    uint8_t *msg_head8 = ip_msg->data;
    uint16_t *msg_head16;
    uint32_t *msg_head32;
    *msg_head8 = head.m_cVersionAndHeaderLen;
    msg_head8++;
    *msg_head8 = head.m_cTypeOfService;
    msg_head8++;
    msg_head16 = msg_head8;
    *msg_head16 = htons( head.m_sTotalLenOfPacket );
    msg_head16++;
    *msg_head16 = htons( head.m_sPacketID );
    msg_head16++;
    *msg_head16 = htons( head.m_sSliceinfo );
    msg_head16++;
    msg_head8 = msg_head16;
    *msg_head8 = head.m_cTTL;
    msg_head8++;
    *msg_head8 = head.m_cTypeOfProtocol;
    msg_head8++;
    msg_head16 = msg_head8;
    *msg_head16 = htons(head.m_sCheckSum);
    msg_head16++;
    msg_head32 = msg_head16;
    *msg_head32 = htonl( head.m_uiSourIp );
    msg_head32++;
    *msg_head32 = htonl( head.m_uiDestIp );
...
struct ofpbuf*
construct_udp_msg( const struct _UDP_HEADER head )
{
    struct ofpbuf *udp_msg = ofpbuf_new( 1536 );
    
    udp_msg->size += 8;
    uint16_t *msg_head = udp_msg->data;
    *msg_head = htons( head.m_usSourPort );
    msg_head++;
    *msg_head = htons( head.m_usDestPort );
    msg_head++;
    *msg_head = htons( head.m_usLength );
    msg_head++;
    *msg_head = htons( head.m_usCheckSum );
...
struct ofpbuf*
construct_pathconfig_msg( const struct ofpbuf* msg, int* len )
{
    struct ofpbuf *pathconfig_msg = ofpbuf_new( 1536 );

    //首先复制openflow头部
    ofpbuf_put( pathconfig_msg, msg->data, 48 );
    //fprintf( fp, "size of pathconfig_msg: %u\n", pathconfig_msg->size );
    //fprintf( fp, "%s\n", ofpbuf_to_string( pathconfig_msg, pathconfig_msg->size ) );
    

    
    //复制match域
    uint8_t *match_length_pointer;
    uint16_t match_length = 0;
    match_length_pointer = msg->data;
    match_length_pointer = match_length_pointer + 50; 
    match_length += *match_length_pointer;
    match_length_pointer++;
    match_length += (*match_length_pointer)*256;
    //fprintf( fp, "the length of match filed is %u\n", *match_length_pointer );
    uint16_t match_len_withpad = ((*match_length_pointer) % 8 == 0) ? (*match_length_pointer) : ((*match_length_pointer)/8+1)*8; 
    //fprintf( fp, "the length of match filed need to be copy %u\n", match_len_withpad );
    
    
    uint8_t *match_head = pathconfig_msg->data;
    uint8_t *match_msg_head = msg->data;
    match_head += 48;
    match_msg_head += 48;
    pathconfig_msg->size += match_len_withpad;
    memcpy( match_head, match_msg_head, match_len_withpad ); 
   // fprintf( fp, "after copy the match, the contents of pathconfig_pkt:%s\n", ofpbuf_to_string( pathconfig_msg, pathconfig_msg->size ) );

    //复制instruction域
    uint8_t *inst_head = match_head + match_len_withpad;
    uint8_t *inst_msg_head = match_msg_head + match_len_withpad;
    uint8_t *inst_len_pointer = inst_head+2;
    uint16_t first_action_len = 0, old_inst_len = 0;

    pathconfig_msg->size += 8;
    memcpy( inst_head, inst_msg_head, 8 ); //复制instruction type及length, 后面需要改length
    inst_head += 8;
    
    inst_msg_head += 2;
    old_inst_len += (*inst_msg_head)*256;
    inst_msg_head++;
    old_inst_len += *inst_msg_head;
    inst_msg_head += 5;
    uint8_t *first_action_len_pointer = inst_msg_head+2;
    first_action_len += (*first_action_len_pointer)*256;
    first_action_len_pointer++;
    first_action_len += (*first_action_len_pointer);
    //fprintf( fp, "length of the first actioin: %u\nlength of the old instruction: %u\n", first_action_len, old_inst_len );
    
    uint16_t action_need_copy_len = old_inst_len - first_action_len - 8;
    inst_msg_head += first_action_len;
    pathconfig_msg->size += action_need_copy_len;
    memcpy( inst_head, inst_msg_head, action_need_copy_len );
   // fprintf( fp, "after copy the actions, the contents of pathconfig_pkt:%s\n", ofpbuf_to_string( pathconfig_msg, pathconfig_msg->size ) );

    //更新instruction的pad以及length
    uint16_t new_inst_len = old_inst_len - first_action_len;
    uint16_t new_inst_len_pad = new_inst_len % 8 == 0 ? new_inst_len : (new_inst_len/8+1)*8; 
    if( new_inst_len_pad != new_inst_len )
        memset( match_head+match_len_withpad+new_inst_len, 0, new_inst_len_pad - new_inst_len );
    uint16_t *new_inst_len_pointer = inst_len_pointer;
    *new_inst_len_pointer = htons( new_inst_len );
    //fprintf( fp, "new inst len: %u, new isnt len with htonl: %u\n", new_inst_len, htons( new_inst_len ) );
    //fprintf( fp, "after update the instruction length, the contents of pathconfig_pkt:\n%s\n", ofpbuf_to_string( pathconfig_msg, pathconfig_msg->size ) );

    //更新openflow length
    uint16_t *openflow_len_pointer = pathconfig_msg->data;
    openflow_len_pointer++;
    uint16_t openflow_len = ntohs( *openflow_len_pointer );
    //fprintf( fp, "old openflow len: %u\n", openflow_len );
    openflow_len = 48 + match_len_withpad + new_inst_len_pad;
    //fprintf( fp, "new openflow len: %u\n", openflow_len );
    *openflow_len_pointer = htons( openflow_len );
    //fprintf( fp, "after update the openflow length, the contents of pathconfig_pkt:\n%s\n", ofpbuf_to_string( pathconfig_msg, pathconfig_msg->size ) );

    *len = openflow_len;
    return pathconfig_msg;
}

void 
send_pathconfig_packet( struct ofproto *ofproto,
                        struct ofputil_flow_mod fm, 
                        const struct ofpbuf* msg, 
                        struct ofpbuf ofpact, 
                        struct ofpbuf remaining_ofpacts )
{
    uint16_t openflow_len;
    struct ofpbuf *pathconfig_msg = construct_pathconfig_msg( msg, &openflow_len );

    struct _UDP_HEADER udp_head;
    udp_head.m_usSourPort = 6653;
    udp_head.m_usDestPort = 318;
    udp_head.m_usLength = openflow_len+8;
    udp_head.m_usCheckSum = 0;
    struct ofpbuf *udp_msg = construct_udp_msg( udp_head );

    struct _IP_HEADER ip_head;
    ip_head.m_cVersionAndHeaderLen = 0x45; //version: 4, header length: 5x4=20
    ip_head.m_cTypeOfService = 0;
    ip_head.m_sTotalLenOfPacket =openflow_len + 8 + 20;
    ip_head.m_sPacketID = 0;
    ip_head.m_sSliceinfo = 0;
    ip_head.m_cTTL = 64;
    ip_head.m_cTypeOfProtocol = 17; //UDP
    ip_head.m_sCheckSum = 0;
    ip_head.m_uiSourIp = 0x01010101;
    ip_head.m_uiDestIp = 0x01010101;
    struct ofpbuf* ip_msg = construct_ip_msg( ip_head );

    struct _MAC_FRAME_HEADER mac_head;
    for( int i = 0; i < 6; i++ )
    {
        mac_head.m_cDstMacAddress[i] = fm.match.flow.dl_dst.ea[i];
        mac_head.m_cSrcMacAddress[i] = fm.match.flow.dl_src.ea[i];
    }
    mac_head.m_cType = 0x0800;
    struct ofpbuf* mac_msg = construct_mac_msg( mac_head );

    struct dp_packet *path_config_packet = dp_packet_new( 1536 );
    dp_packet_put( path_config_packet, mac_msg->data, 14 );
    dp_packet_put( path_config_packet, ip_msg->data, 20 );
    dp_packet_put( path_config_packet, udp_msg->data, 8 );
    dp_packet_put( path_config_packet, pathconfig_msg->data, openflow_len );

    struct rule* path_config_rule = NULL;
    enum ofperr error = fastlane_handle_flow_mod__( ofproto, &fm, &path_config_rule );

    fastlane_run_rule_executes( ofproto, path_config_rule, &(fm.match.flow), path_config_packet );
...
static enum ofperr
handle_flow_mod(struct ofconn *ofconn, const struct ofp_header *oh, const struct ofpbuf* msg)
    OVS_EXCLUDED(ofproto_mutex)
{
    ...
    uint64_t ofpacts_stub[1024 / 8];
    struct ofpbuf ofpacts;
    struct ofpbuf remaining_ofpacts; //to get the rest actions in flowmod'
    enum ofperr error;
    ...
    error = fastlane_ofputil_decode_flow_mod(&fm, oh, ofconn_get_protocol(ofconn),
                                             &ofpacts,
                                             u16_to_ofp(ofproto->max_ports),
                                             ofproto->n_tables,
                                             &remaining_ofpacts);

    if( remaining_ofpacts.size != 0  )
    {
        uint8_t *type;
        type = msg->data;
        type = type+25;
        if( *type == 0 ) //type is add
            send_pathconfig_packet( ofproto, fm, msg, ofpacts, remaining_ofpacts );
    }
    

    if (!error) {
        struct openflow_mod_requester req = { ofconn, oh };
        fm.match.wc.masks.in_port.odp_port = 0; 
        error = handle_flow_mod__(ofproto, &fm, &req);
    }

    ofpbuf_uninit(&ofpacts);
    ofpbuf_uninit(&remaining_ofpacts);
    return error;
}


enum ofperr
fastlane_handle_flow_mod( void *ofconn_void,  const struct ofpbuf* msg)
{
    struct ofconn* ofconn = ofconn_void;
    struct ofp_header *oh = msg->data;
    return handle_flow_mod( ofconn, oh, msg );
}
...
static enum ofperr
fastlane_handle_flow_mod__( struct ofproto *ofproto, const struct ofputil_flow_mod *fm, 
                            struct rule** new_rule )
    OVS_EXCLUDED(ofproto_mutex)
{
    struct ofproto_flow_mod ofm;
    ...
    error = fastlane_ofproto_flow_mod_init(ofproto, &ofm, fm, new_rule);
    if (error) {
        return error;
    }
...
static enum ofperr
fastlane_ofproto_flow_mod_init(struct ofproto *ofproto, struct ofproto_flow_mod *ofm,
                      const struct ofputil_flow_mod *fm, struct rule** new_rule)
    OVS_EXCLUDED(ofproto_mutex)
{
    enum ofperr error;
    ...
    error = fastlane_add_flow_init(ofproto, ofm, fm, new_rule);
